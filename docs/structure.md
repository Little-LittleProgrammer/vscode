...
## 技术架构
![技术架构](./images/main-2.png)
该应用程序分为几个进程，每个进程都有特定的职责：

- 主进程：协调应用程序生命周期并管理窗口
- 渲染器进程：处理 UI 和工作台组件
- 扩展主机进程：独立运行扩展
- 共享进程：处理跨窗口共享的资源密集型操作
- 工作进程 (Worker Processes)：处理计算密集型任务，如文本搜索、Git操作等
- 语言服务进程：运行语言服务器协议（LSP）实现，如 TypeScript/JavaScript、Python、C++ 等的智能补全、诊断等。


### 详细
VSCode 采用了多进程+多线程的架构，核心目的是提升性能、稳定性和安全性。主要进程类型如下：
1. 主进程（Main/Node Main/主控进程）
职责：负责窗口管理、菜单、原生对话框、与操作系统交互、进程调度等。
技术栈：Node.js 环境，运行 Electron 的主进程代码。
特点：只负责管理和调度，不直接渲染 UI。
2. 渲染进程（Renderer/窗口进程）
职责：负责 UI 渲染、用户交互、前端逻辑。
技术栈：Chromium（即浏览器内核），运行 HTML/CSS/JS。
特点：每个 VSCode 窗口对应一个渲染进程，互相隔离。
3. 扩展主机进程（Extension Host）
职责：运行所有第三方/内置扩展的代码（如 TypeScript 插件、Git 插件等）。
技术栈：Node.js 环境，沙箱隔离。
特点：与主进程、渲染进程通过 IPC（进程间通信）协议通信，扩展之间共享同一个进程（也有多扩展主机的情况）。
4. 语言服务进程（Language Server Process）
职责：运行语言服务器协议（LSP）实现，如 TypeScript/JavaScript、Python、C++ 等的智能补全、诊断等。
技术栈：Node.js 或独立可执行文件。
特点：通常每种语言一个进程，和主进程/扩展主机通过 RPC 通信。
5. 其他辅助进程
如：终端进程（Pty Host）、搜索进程、调试适配器进程等，按需启动，负责特定功能。
