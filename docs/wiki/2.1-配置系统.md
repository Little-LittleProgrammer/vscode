...
```mermaid
flowchart TD
    WCMP["WorkspaceConfigurationModelParser"] -->|"继承"| CMP["ConfigurationModelParser"]
    SCMP["StandaloneConfigurationModelParser"] -->|"继承"| CMP

    WCMP -->|"解析"| WF["Workspace file"]
    SCMP -->|"解析"| WF
    SCMP -->|"解析"| SCF["Standalone config file(tasks, launch)"]

    CMP -->|"创建"| CM["ConfigurationModel"]
    CMP -->|"解析"| JSF["JSON settings file"]

    CM -->|"存储"| CV["Contents (settings values)"]
    CM -->|"跟踪"| KS["Keys (setting names)"]
    CM -->|"管理"| OS["Overrides (language-specific settings)"]
```
